...
tape( 'the function has an arity of 1', function test( t ) {
	t.strictEqual( gsumkbn2.length, 1, 'has expected arity' );
	t.end();
});
...
tape( 'the function computes the sum of all elements in a one-dimensional ndarray', function test( t ) {
	...
	v = gsumkbn2( [ vector( x, 6, 1, 0 ) ] );
	t.strictEqual( v, 3.0, 'returns expected value' );

	x = [ -4.0, -5.0 ];
	v = gsumkbn2( [ vector( x, 2, 1, 0 ) ] );
	t.strictEqual( v, -9.0, 'returns expected value' );

	x = [ -0.0, 0.0, -0.0 ];
	v = gsumkbn2( [ vector( x, 3, 1, 0 ) ] );
	t.strictEqual( isPositiveZero( v ), true, 'returns expected value' );

	x = [ NaN ];
	v = gsumkbn2( [ vector( x, 1, 1, 0 ) ] );
